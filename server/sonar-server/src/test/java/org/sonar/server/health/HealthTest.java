...
public class HealthTest {
  ...
  private final Health.Status randomStatus = Health.Status.values()[random.nextInt(Health.Status.values().length)];
  ...
  public void getStatus_returns_status_from_builder() {
    Health underTest = newHealthCheckBuilder().setStatus(randomStatus).build();
  ...
  public void equals_is_based_on_status_and_causes() {
    ...
    builder1.setStatus(randomStatus);
    builder2.setStatus(randomStatus);
  ...
  public void verify_toString() {
    ...
    AbstractCharSequenceAssert<?, String> a = assertThat(underTest)
      .describedAs("toString for status %s and causes %s", randomStatus, randomCauses);
    if (randomCauses.isEmpty()) {
      a.isEqualTo("Health{" + randomStatus + ", causes=[]}");
    } else if (randomCauses.size() == 1) {
      a.isEqualTo("Health{" + randomStatus + ", causes=[" + randomCauses.iterator().next() + "]}");
    } else {
      a.startsWith("Health{" + randomStatus + ", causes=[")
        .endsWith("]}")
