...
public class SetAction implements SettingsWsAction {
  ...
  private void commonChecks(SetRequest request, Optional<ComponentDto> component) {
    ...
    component.map(ComponentDto::getBranch).ifPresent(b -> checkArgument(SettingsWs.SETTING_ON_BRANCHES.contains(settingKey), format("Setting '%s' cannot be set on a branch", settingKey)));
  }
